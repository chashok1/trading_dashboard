...
with session_scope() as session:
    # Check if source_file is populated for hist_tw 2026-05-27
    ...
    sample = session.execute(text("""
        SELECT symbol, source_file FROM hist_tw WHERE snapshot_date = '2026-05-27' LIMIT 3
    """)).fetchall()
    print(f"  Sample:")
    for row in sample:
        print(f"    {row[0]}: {row[1]}")
