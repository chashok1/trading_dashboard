...
def load_tw(session: Session, wb: Workbook, source_file: str) -> tuple[int, int, int]:
    ...
    sheet_name = get_sheet_case_insensitive(wb, "TW")
    if sheet_name is None:
        if len(wb.sheetnames) == 1:
            sheet_name = wb.sheetnames[0]
        else:
    ...
    if not records:
        print(f"  [TW->hist_tw] All rows skipped.")
        return rows_read, 0, rows_read

    def _progress(batch, total, ins, skp):
        print(f"  [TW->hist_tw] batch {batch}/{total} ({int(100*batch/total)}%) cumulative: {ins} inserted, {skp} skipped")

    n_attempted, n_inserted = insert_upsert(session, "hist_tw", records, progress_cb=_progress)
    n_skipped = n_attempted - n_inserted

    print(f"  [TW->hist_tw] DONE  read={rows_read}  inserted={n_inserted}  skipped={n_skipped}")
