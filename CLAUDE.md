...
| Watch/notify (🔔 column) — same-day price/level alert, live-joined (NOT baked into drv_actionable, unlike LT above); triggers on a % move or a genuine crossing of LRR/TRR/Trade line/Trend line/a $ target vs. its baseline-at-creation reading, or a plain no-condition reminder | `ref_watch` (`db/baseline.sql`'s own comment has the full design) → live `LEFT JOIN LATERAL` in `get_actionable` (`api/routers/dash.py`); `GET/POST/PATCH/DELETE /api/actionable/watch`; background check + once-daily combined pre-close email digest in `etl/derive_watch.py`, called from `etl/scheduler.py`'s `maybe_check_watches`/`maybe_send_watch_digest` (`ref_settings.watch_digest_hour`, default 15); grid badge + quick-add popover + toolbar "Watching" panel in `web/actionable.js`/`web/actionable.html` |
...
| Hourly intraday Yahoo price-only refresh (market hours 9:30-4 ET, cache-only, no hist_y row, skips if EOD Yahoo job running/done; also gates PVV recompute — see `docs/pvv_logic.md`) | `etl/yahoo_fetch.py::fetch_hourly_quotes`; `etl/scheduler.py::maybe_run_hourly_quote_refresh` |
| Symbol export-frequency tier (user_pinned / held / active_90d / hedgeye_90d / dashboard_dependency / special_format / dormant → tier 1=daily, 2=weekly; universe = `ref_my_stocks` active list ∪ held ∪ dashboard-dependency ∪ user-pinned, not raw `drv_symbols`) | `etl/derive_symbol_tier.py`; `drv_symbol_tier` |
| Daily TOS watchlist file generation (WL1-13=tier1/daily, WL14+=tier2/weekly, stable symbol→WL persisted in `ref_watchlist_assignment`, writes WL\*.csv + additions.csv + overflow.csv) — feeds `TOSDownloads/LoadWatchlists.py` (full re-import) + `TOSDownloads/ImportAdditions.py` (delta-only import), both driven by `TOSDownloads/EditWatchlist.csv` + `TOSDownloads.py::edit_watchlist()` | `etl/generate_watchlist_files.py`; event-driven — fires from `etl/etl_load.py::load_one_file()` right after a TOSD/TOSL/TOSW load derives successfully (`daily` mode only — `weekly` mode not yet scheduled, run manually: `python -m etl.generate_watchlist_files --mode weekly`); output dir = `settings.watchlist_files_dir` |
| Per-account TOS + Yahoo watchlist files (held positions only, one `<short_name>.csv` per active `ref_accounts` row, grouped by `short_name` so a broker-renamed account label still consolidates to one file) | `etl/generate_account_watchlist_files.py`; same trigger as the row above (`etl_load.py`, right after it); output dirs = `settings.account_tos_watch_lists_dir` / `settings.account_y_watch_lists_dir` |
| Per-source TOS + Yahoo watchlist files (PS/ETF/II/RR/CALL/SSS — Bullish-only via `drv_outlooks` for RR/CALL/ETF/II, all-ranked via raw `hist_ps`/`hist_sss` latest snapshot for PS/SSS; Yahoo file translates macro/FX/futures `tos_symbol`s via `ref_rrt.y_ticker`) | `etl/generate_source_watchlist_files.py`; same trigger as the two rows above (`etl_load.py`, right after them); output dirs = `settings.source_tos_watch_lists_dir` / `settings.source_y_watch_lists_dir` |
...
| Market Read (theme map, source breadth, SSS sector rows+books, theme grid on `/`, Risk Dial positioning gauges, validation scorecards) | `docs/market_state_factor_sss_design.md`; `etl/derive_market_read.py`, `etl/derive_sss_breadth.py` → `drv_source_breadth`/`drv_theme_stance`/`drv_sss_breadth`; `GET /api/market-read`, `GET /api/market-read/sectors` (`api/routers/cockpit.py`); `web/market_read.js` (`#quadRotationPanel`, retired the Quad Rotation tiles); validation report `docs/audit/market_read_validation_2026-09.md` |
...
| MACRO column sector/asset-class/style dots (per-membership live quad-regime stance, 2026-08-01) | `etl/derive_macro.py::_window_stance_for` → `drv_macro_score.{sector_stance,asset_class_stance,style_stances}`; `web/actionable.js::_macroMemberBarsHtml`. Styles kept as a JSONB array (not averaged) — a symbol can carry several independent style tags that disagree with each other. |
