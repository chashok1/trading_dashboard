...
| Daily TOS watchlist file generation (WL1-13=tier1/daily, WL14+=tier2/weekly, stable symbol→WL persisted in `ref_watchlist_assignment`, writes WL\*.csv + additions.csv + overflow.csv) — feeds `TOSDownloads/LoadWatchlists.py` (full re-import) + `TOSDownloads/ImportAdditions.py` (delta-only import), both driven by `TOSDownloads/EditWatchlist.csv` + `TOSDownloads.py::edit_watchlist()` | `etl/generate_watchlist_files.py`; rides `etl/scheduler.py`'s nightly job (`daily` mode only — `weekly` mode not yet scheduled, run manually: `python -m etl.generate_watchlist_files --mode weekly`); output dir = `settings.watchlist_files_dir` |
